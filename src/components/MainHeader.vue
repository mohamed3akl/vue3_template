      ...
      <v-container>
        ...
        <v-row class="align-center">
          <a href="/" class="logo me-10">
            <img :src=" getAdminSettings.logo_path ||'@/assets/img/geneoLogo.png'" width="130"
                height="68" alt="Genom" />
          </a>
          <div class="search" v-if="AuthedUser.user_type == 1">
            <input
              type="text"
              :placeholder="
                $vuetify.locale.t(
                  '$vuetify.Search_for_the_patient_name_or_reservation_number_you_want_to_search_for'
                )
              "
            />
            <img src="@/assets/img/icons/svg/search.svg" />
          </div>
          <location-switch class="ms-auto me-10" />
          <lang-switch />

          <a
            href="/booking/genetic-service"
            class="btn-href ms-10"
            v-if="AuthedUser.user_type == 0"
          >
            {{ $vuetify.locale.t("$vuetify.genetic_service") }}
          </a>
        </v-row>
      </v-container>
    </v-row>
    <v-row class="bottomBar">
      <v-container>
        <v-row class="align-center">
          <div class="d-flex w-100 align-center justify-space-between">
            <ul class="nav">
              ...
              <li :class="$route.name == 'About' ? 'me-10 active' : 'me-10'">
                <a href="/about">{{ $vuetify.locale.t("$vuetify.who_we_are") }}</a>
              </li>
              <li
                :class="
                  $route.name == 'HelpCenter' && $route.query.tag == 'question'
                    ? 'me-10 active'
                    : 'me-10'
                "
              >
                <router-link
                  :to="{ name: 'HelpCenter', query: { tag: 'question' } }"
                  >{{ $vuetify.locale.t("$vuetify.faqs") }}</router-link
                >
              </li>
              <li
                :class="
                  $route.name == 'HelpCenter' &&
                  ($route.query.tag == 'contact' || !$route.query.tag)
                    ? 'me-10 active'
                    : 'me-10'
                "
              >
                <router-link
                  :to="{ name: 'HelpCenter', query: { tag: 'contact' } }"
                  >{{ $vuetify.locale.t("$vuetify.contact_us") }}</router-link
                >
              </li>
            ...
            <ul class="actions_nav">
              ...
              <li class="ms-8">
                <a href="/my-profile">
                  <img
                    src="@/assets/img/icons/svg/Lock.svg"
                    v-if="$route.name != 'ShowProfile'"
                  />
                  <img
                    src="@/assets/img/icons/svg/Lock-active.svg"
                    v-if="$route.name == 'ShowProfile'"
                  />
                </a>
              </li>
              ...
              <li class="ms-8">
                <a href="/settings">
                  <img
                    src="@/assets/img/icons/svg/Setting.svg"
...
export default {
  ...
  mounted() {
    ...
    console.log(this.getAdminSettings)
  },
  computed: {
    ...mapGetters(["AuthedUser","getAdminSettings"]),
  },
};
</script>
